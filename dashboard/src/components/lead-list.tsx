...
import { formatDate, isLeadStatus, statuses } from '@/lib/leads';
export function StatusBadge({ status }: { status: string }) {
  return <span className={'badge status-' + status.toLowerCase()}>{isLeadStatus(status) ? statuses[status] : 'Sin clasificar'}</span>;
}
export default function LeadList({ leads, filtered = false }: { leads: Omit<Lead, 'lastHistory'>[]; filtered?: boolean }) {
  ...
  return <div className="table-wrap"><table><thead><tr><th>Contacto</th><th>Interés / análisis</th><th>Estado</th><th>Detectado</th><th><span className="sr-only">Detalle</span></th></tr></thead><tbody>
    {leads.map(lead => <tr key={lead.id}>
      ...
      <td><StatusBadge status={lead.status} /></td><td className="muted nowrap">{formatDate(lead.createdAt)}</td>
